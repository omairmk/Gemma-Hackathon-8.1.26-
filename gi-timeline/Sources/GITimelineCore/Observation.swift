...
/// The one-to-one Bristol/form contract used by parsing, evaluation, and
/// route-specific postconditions. A null Bristol type is intentionally not
/// coerced to the nearest form: only an explicit mixed observation or a full
/// abstention is valid without a type.
public enum BristolFormContract {
  public static let expectedForms: [Int: String] = [
    1: "hard_lumps",
    2: "lumpy_formed",
    3: "cracked_formed",
    4: "smooth_formed",
  ...
  public static func expectedForm(for bristolType: Int) -> String? {
    expectedForms[bristolType]
  }

  public static func isValid(bristolType: Int?, form: String) -> Bool {
    if let bristolType {
      return expectedForm(for: bristolType) == form
    }
    return form == "mixed" || form == "unable_to_assess"
  }

  public static func validate(bristolType: Int?, form: String) throws {
    guard isValid(bristolType: bristolType, form: form) else {
      if let bristolType, let expected = expectedForm(for: bristolType) {
        throw ObservationParserError.inconsistent("Bristol type \(bristolType) requires form \(expected).")
      }
      if bristolType == nil {
        throw ObservationParserError.inconsistent("Null Bristol type requires mixed or unable_to_assess form.")
      }
      throw ObservationParserError.invalidValue("apparent_bristol_type")
    }
  }
}
...
public enum ObservationParser {
  ...
  public static func validate(_ observation: VisualObservation) throws {
    ...
    try BristolFormContract.validate(bristolType: observation.apparentBristolType, form: observation.form)
