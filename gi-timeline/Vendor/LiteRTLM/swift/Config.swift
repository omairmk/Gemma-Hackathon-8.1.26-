...
public enum Backend: Equatable, Hashable, Sendable {
  ...
  var cpuThreadCount: Int? {
    guard case .cpu(let threadCount) = self else { return nil }
    return threadCount
  }
}
...
public struct EngineConfig: Hashable, Sendable {
  ...
  /// Maximum number of images accepted by the engine. When `nil`, use the
  /// runtime default. The GI Journal raw-photo route sets this explicitly.
  ...
  ///     model or the engine.
  ///   - maxNumImages: Maximum number of images accepted by the engine. A value
  ///     of zero disables image capacity; `nil` uses the runtime default.
  ...
  /// - Throws: `LiteRTLMError` if token/image limits or an explicit main CPU
  ///   thread count are outside their reviewed native ranges.
  public init(
    ...
    if let maxNumImages,
      maxNumImages < 0 || Int32(exactly: maxNumImages) == nil
    {
      throw LiteRTLMError.config(.invalidMaxNumImages(count: maxNumImages))
    }
    if let threadCount = backend.cpuThreadCount,
      threadCount <= 0 || Int32(exactly: threadCount) == nil
    {
      throw LiteRTLMError.config(.invalidCPUThreadCount(count: threadCount))
    }
