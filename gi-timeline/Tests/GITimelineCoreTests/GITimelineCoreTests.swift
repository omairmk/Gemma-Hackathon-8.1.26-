...
final class GITimelineCoreTests: XCTestCase {
  ...
  func testSafetyRulesTruthTable() {
    ...
    for redBlood in values { for blackTarry in values { for dizziness in values { for severePain in values {
      XCTAssertEqual(
        ...
        SafetyRules.guidance(redBlood: redBlood, blackTarry: blackTarry) != .none
      )
    } } } }
  }
  ...
  func testTimeoutCanRevokeHungAttemptAndEnterRetainedDraftManualEntry() {
    var state = NewEntryWorkflow()
    let prep = state.beginPreparation()
    _ = state.finishPreparation(prep, prepared: .init(path: "/draft", sha256: "hash"))
    let attempt = state.beginAttempt()!

    XCTAssertTrue(state.timeoutAttemptAndBeginManualEntry(attempt, keepingDraft: true))
    XCTAssertFalse(state.isLocked)
    XCTAssertTrue(state.manualEntryActive)
    XCTAssertEqual(state.draft?.path, "/draft")
    XCTAssertTrue(state.canSave)
    XCTAssertFalse(state.canAnalyze)
    XCTAssertFalse(state.finishAttempt(attempt), "A late completion must stay stale")
  }
