...
enum PrivacyDiagnostics {
  ...
  static func assertCompleteProtection(at urls: [URL], requireExisting: Bool = true) {
    ...
    // The simulator filesystem does not report iPhone data-protection classes.
    // Keep this strict assertion active on physical devices, which remain the
    // authority for the privacy gate.
